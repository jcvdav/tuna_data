...
ggplot(ps_ts, aes(year, catch, color = dataset)) +
...
ggplot(ll_ts, aes(year, catch, color = dataset)) +
...
ggplot(month_1deg_ps, aes(x = year, fill = rfmo)) +
...
```{r}
#| code-fold: true
#| code-summary: "Show code for creating species catch time series"
#| echo: true
#| warning: false
#| message: false


### Creating a purse seine species catch time series ############################
...
ggplot(year_1deg_ps, aes(x = year, fill = rfmo)) +
    geom_bar(color = "white", linewidth = 0.2) +
