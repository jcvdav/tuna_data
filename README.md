...
This section explains how the repository is structured and key formats that must
be preserved when modifying the code.
...
the same cell at the same time) must be detected.
- Overlap detection scripts should be stored in `02_processing/01_bind_data`.
